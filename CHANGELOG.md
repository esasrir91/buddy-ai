...
- **Proxy-aware caching (`proxy_type`)** on `OpenAIChat` — when routing Claude or other models through an LLM proxy, set `proxy_type="litellm"`, `"openrouter"`, or `"azure"` to automatically inject the proxy's native cache headers. Works alongside `cache_prompt=True` for a full two-layer cache stack.
- **Token Budget Manager (`TokenBudgetConfig`)** — attach to any `Model` or `Agent` to enforce a pre-invoke token ceiling, warn at configurable thresholds, auto-drop oldest history turns until under budget, and cap verbose tool results. Budget metrics (estimated tokens, turns dropped, tokens trimmed) surface in `RunResponse.extra_data["token_budget"]`. New module `buddy/models/token_budget.py`.
- **Automatic tool-result trimming** — when `TokenBudgetConfig.max_tool_result_tokens` is set, oversized tool outputs are truncated before they reach the model, with a brief notice appended. Integrated into `Model.format_function_call_results()`.
- **Token efficiency docs** — new `docs/advanced/token-efficiency.md` covering all six techniques (prompt caching, proxy caching, token budget, tool trimming, history truncation, session summaries) with a proxy decision tree and cost impact table.
