...
def train_model(
    ...
    try:
        ...
        trainer = ModelTrainer(base_model=model, output_dir=output_dir)
        ...
        for model_name in os.listdir(DEFAULT_MODELS_DIR):
            ...
            if os.path.isdir(model_path):
                if os.path.exists(metadata_path):
                    ...
                    models.append(
                        {"name": model_name, "description": "Legacy model (no description)", "created_at": "Unknown"}
                    )
...
def use_with_agent(model_name: str, agent_name: str = "My Custom Agent", instructions: str = None):
    ...
    try:
        ...
        trained_model = create_trained_model(model_path=model_path, model_name=f"Trained Model: {model_name}")
        ...
        agent = Agent(name=agent_name, model=trained_model, instructions=instructions or default_instructions)
