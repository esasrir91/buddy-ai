...
class Model(ABC):
    ...
    # --- Prompt Caching ---
    # Set cache_prompt=True to enable provider-native prompt caching.
    # For Anthropic: injects cache_control breakpoints on system / tools / history.
    # For OpenAI: server-side caching is automatic (>=1024 token prefix); enabling
    # this flag surfaces per-request cache-hit metrics in RunResponse.
    ...
    def __post_init__(self):
        ...
        if self.cache_prompt and self.prompt_cache_config is None:
            from buddy.models.cache import PromptCacheConfig

            self.prompt_cache_config = PromptCacheConfig(enabled=True)
