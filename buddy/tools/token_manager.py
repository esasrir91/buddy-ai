...
from buddy.utils.log import log_debug, log_error, log_info, log_warning, logger
...
class TokenManager(Toolkit):
    ...
    def register_with_agent(self, agent=None) -> str:
        ...
        if agent is not None:
            ...
            if success:
                ...
                return json.dumps({"status": "error", "message": "Failed to register with agent"})
        else:
    ...
    def _check_limits(self, input_tokens: int, output_tokens: int = 0) -> Dict[str, Any]:
        ...
        elif self.limits.max_input_tokens and input_tokens > (
            self.limits.max_input_tokens * self.limits.soft_limit_threshold
        ):
        ...
        if self.limits.max_tokens_per_request and total_tokens > self.limits.max_tokens_per_request:
            violations.append(
                f"Request tokens ({total_tokens}) exceed per-request limit ({self.limits.max_tokens_per_request})"
            )

        if self.limits.max_total_tokens and (self.stats.total_tokens + total_tokens) > self.limits.max_total_tokens:
            violations.append(
                f"Session total ({self.stats.total_tokens + total_tokens}) would exceed session limit ({self.limits.max_total_tokens})"
            )
    ...
    def reset_usage_stats(self) -> str:
        ...
        """
        try:
            old_stats = {"total_tokens": self.stats.total_tokens, "session_count": self.stats.session_count}
        ...
        try:
            ...
            result = {
                ...
                "remaining_session_tokens": (
                    self.limits.max_total_tokens - self.stats.total_tokens if self.limits.max_total_tokens else None
                ),
            }
    ...
    def estimate_cost(self, text: str, model: str = "gpt-4", expected_output_tokens: int = 0) -> str:
        ...
        try:
            ...
            result = {
                ...
                "cost_per_1k_tokens": {"input": model_costs["input"], "output": model_costs["output"]},
            }
    ...
    def suggest_optimal_chunks(self, text: str, target_chunk_tokens: Optional[int] = None) -> str:
        ...
        try:
            ...
            estimated_chunks = max(
                1, (total_tokens // target_chunk_tokens) + (1 if total_tokens % target_chunk_tokens > 0 else 0)
            )
            ...
            result = {
                ...
                "analysis": {
                    ...
                    "average_chunk_tokens": (
                        sum(chunk["tokens"] for chunk in suggested_chunks) // len(suggested_chunks)
                        if suggested_chunks
                        else 0
                    ),
                },
