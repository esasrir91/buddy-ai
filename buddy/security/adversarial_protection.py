...
class PrivacyViolationDetector(ThreatDetector):
    ...
    def _load_pii_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "ssn": {"pattern": r"\b\d{3}-\d{2}-\d{4}\b", "severity": "critical", "confidence": 0.95},
            "credit_card": {"pattern": r"\b(?:\d{4}[-\s]?){3}\d{4}\b", "severity": "critical", "confidence": 0.85},
            "address": {
...
class BehavioralAnalysisEngine:
    ...
    def analyze_behavior(
        self, user_id: str, session_id: str, input_text: str, context: Dict[str, Any] = None
    ) -> List[SecurityThreat]:
    ...
    def _update_session_data(self, session_id: str, input_text: str, context: Dict[str, Any]):
        ...
        session["inputs"].append({"text": input_text, "timestamp": datetime.now(), "length": len(input_text)})

        # Check for rapid requests
        now = datetime.now()
        recent_requests = [inp for inp in session["inputs"] if (now - inp["timestamp"]).total_seconds() < 10]
...
class ContentSanitizer:
    ...
    def _load_sanitization_rules(self) -> Dict[str, Any]:
        ...
        return {"preserve_meaning": True, "aggressive_filtering": False, "allow_partial_sanitization": True}
...
class AdversarialProtectionSystem:
    ...
    def __init__(self, config: Optional[SecurityConfig] = None):
        ...
        self.detectors = [PromptInjectionDetector(), HarmfulContentDetector(), PrivacyViolationDetector()]
        ...
        self.rate_limiter = RateLimiter(self.config.max_requests_per_minute, self.config.max_requests_per_hour)
    ...
    def analyze_input(
        ...
        if user_id and session_id and self.config.behavioral_analysis_enabled:
            try:
                behavioral_threats = self.behavioral_analyzer.analyze_behavior(user_id, session_id, input_text, context)
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_events": len(
                [e for e in self.security_events if (datetime.now() - e.timestamp).total_seconds() < 3600]
            ),
        }
    ...
    def _log_security_event(self, event: SecurityEvent):
        """Log security event"""
        threat_summary = ", ".join([f"{t.threat_type.value}({t.threat_level.value})" for t in event.threats_detected])
...
class RateLimiter:
    ...
    def allow_request(self, user_id: str) -> bool:
        ...
        recent_minute_requests = sum(1 for timestamp in user_history if timestamp > minute_ago)
...
class AdversarialProtectionMixin:
    ...
    def process_with_security(
        ...
        return self.protection_system.analyze_input(
            user_input, user_id=self.user_id, session_id=self.session_id, context=context
        )
